...
// centered search, bell (unread dot), sign-out, sage primary CTA
// (Dashboard → "Open Live Wall").
const ICON_BUTTON =
    ...
    <header className="flex items-center gap-4 px-6 pb-2 pt-6 lg:px-8">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2.5 md:ml-0">
        <button type="button" aria-label="Notifications" className={`relative ${ICON_BUTTON}`}>
          ...
          {hasUnread && (
            <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-coral" aria-hidden />
          )}
        </button>
        <button type="button" aria-label="Sign out" className={ICON_BUTTON}>
